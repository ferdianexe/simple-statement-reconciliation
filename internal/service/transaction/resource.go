...
//go:generate mockgen -source=resource.go -destination=resource_mock.go -package=transaction
...
type csvRepoProvider interface {
	...
	ParseSystemTransactions(ctx context.Context, path string, start, end time.Time) ([]csv.Transaction, error)
}

// Resource type of a transaction resource.
...
// NewResource initiates new transaction resource.
